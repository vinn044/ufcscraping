...
from parlay import generate_parlays, parlay_probability
...
def find_closest_fighter_name(name):
    ...
    all_names = pd.concat([df["r_name"], df["b_name"]]).dropna().unique()

    exact_matches = [fighter for fighter in all_names if fighter == name]
    ...
    if close_matches:
        ...
        answer = input(f'Fighter not found. Did you mean "{suggestion}"? y/n: ')

        if answer.lower() == "y":
            return suggestion

    raise ValueError(f"Fighter not found in dataset: {name}")
...
def get_fighter_stats(name):
    ...
    fighter = pd.concat([red_fights, blue_fights])
    ...
    if any(pd.isna(value) for value in stats.values()):
        raise ValueError(f"Missing stats for fighter: {name}")
...
choice = input("Choose mode: 1 = Single fight, 2 = Parlays: ")

if choice == "1":
    fighter_one = input("Pick first fighter: ")
    fighter_two = input("Pick second fighter: ")

    fighter_one_prob, fighter_two_prob = predict_fight(
        fighter_one,
        fighter_two
    )

    print(f"{fighter_one}: {fighter_one_prob * 100:.2f}%")
    print(f"{fighter_two}: {fighter_two_prob * 100:.2f}%")

elif choice == "2":
    number_of_fights = int(input("How many fights are you picking from? "))
    parlay_size = int(input("How many legs per parlay? "))
    ...
    for i in range(number_of_fights):
        ...
        if fighter_one_prob > fighter_two_prob:
            picks.append((fighter_one, fighter_one_prob))
        else:
            picks.append((fighter_two, fighter_two_prob))

    parlays = generate_parlays(picks, parlay_size)

    print("\nGenerated Parlays:")

    for parlay in parlays:
        names = [fighter for fighter, probability in parlay]

        combined_probability = parlay_probability(parlay)
